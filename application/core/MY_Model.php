...
class General_model extends My_model {
    ...
    */
    public function update_or_insert($data = array(), $primary_key = array(), $no_primary_key = array()) {
    	...
    	$pk = array_columns($data, $primary_key);
		...
		$repeated_pk = $this->db->query("SELECT CONCAT_WS('&', $string_pk) as PK FROM centros_costo WHERE CONCAT_WS('&', $string_pk) IN('$string_data_pk')")->result();
    ...
    public function get($select = '*', $where = array(), $where_in = array(), $where_not_in = array(), $order_by = 'id DESC') {
    	if(!empty($where)) {
    		$this->db->where($where);
    	}
    ...
    public function simple_data_table($columns = array(), $primaryKey = 'id') {
		// DB table to use
		$table = $this->table;
