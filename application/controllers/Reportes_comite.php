...
class Reportes_comite extends CI_Controller {
    private $data = array();
    ...
    //VIEW_HANDLER
        $this->view_handler->view('comite', 'Reportes', $this->data);
    }
    ...
    public function downloadcarac($id = null){
        ...
        $filepath = './files/Reportes/crtz/Comite/procedimientos '.$id.' '.$ruta[0].' '.$ruta[1].'.pdf';
    ...
    public function Derivacion(){
        $id_caso = $this->input->post('$id');
        if($this->ion_auth->in_group(6)){
            $data = $this->Rep_to_derivacion_model->Derivacionotros($id_caso);
            echo json_encode($data);
            return;
        }
    }
    ...
    public function downloadderiv($id = null){
        ...
        $filepath = './files/Reportes/drvz/comite/Derivación '.$id.' '.$ruta[0].' '.$ruta[1].'.pdf';
        force_download($filepath, NULL);
    }
}