	...
	class PerfilUseralumno extends CI_Controller {
		private $data = array();
	    ...
	    public function __construct(){
			...
			$this->data['breadcrumb'] = array(
				array(
					...
					'link' =>  site_url('perfiluseralumno/index')
				)
			);
		...
		public function index() {
			...
			*/
			...
			$this->data['direccion'] = $this->casos_model->address($this->session->userdata('user_id'));
			...
			if($this->ion_auth->in_group(array(3,7,9))) $this->data['curso'] = $this->Cursos_model->get('*', array('id' => $this->session->userdata('sector')))[0]->nombre;
				else $this->data['curso'] = "No Aplica";
			
			$this->view_handler->view('perfilUseralumno', 'perfil', $this->data);
		}
		...
		public function ajustes(){
			...
			if($this->input->post('finalizar') == 1){
				...
				if($this->form_validation->run()){
					...
					$this->casos_model->actualizar($user,$data);
					$hoy = date('Y-m-d');
					$id_caso = $this->casos_model->casoporid($this->session->userdata('user_id'));
					$caso = $this->casos_model->caso($id_caso);
					$this->Address_model->address($id_caso, $data, $hoy);

					$direcciones = $this->Address_model->getaddress($id_caso);
					$fechas = $this->Address_model->getfechas($id_caso);
					$this->createpdf($caso, $direcciones, $fechas);
					...
					if ($this->ion_auth->update($user, $data))
					{
						...
						redirect('perfilUseralumno', 'refresh');
			...
			$this->view_handler->view('perfilUseralumno', 'ajustes', $this->data);
		}

	public function createpdf($caso = null, $direcciones = null, $fechas = null){
		require_once("files/Reportes/plantillas/Reporte/georef.php");
		date_default_timezone_set("America/Santiago");
		$hoy = date('Y-m-d');
		$html = getPlantilla($caso, $direcciones, $fechas, $hoy);
    	$pdfFilePath = 'Indicador de georeferencia '.$caso[2].' '.$caso[3].' '.$caso[4].' '.$hoy.'.pdf';
