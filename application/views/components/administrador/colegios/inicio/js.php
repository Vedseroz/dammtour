...
$(document).ready(function() {
	...
	.DataTable( {
        ...
        "serverSide": true,
		"ajax": {
            "url": "<?= site_url('Administrador/ajaxColegios') ?>",
            "type": "GET"
        },
        "columnDefs": [
            {
            ...
            },
            {
	            ...
	            "render": function ( data, type, row ) {
	            	...
	            	var link_edit = '<?= site_url('Administrador/edit_colegio') ?>' + '/' + row.id;
                    var link_detalles = '<?= site_url('Administrador/show_colegio') ?>' + '/' + row.id;

                    var remove_normal = '<a class="red" title="Eliminar" href="#" data-toggle="modal" data-target="#deleteModal" data-id="' + row.id + '" data-codigo="' + row.codigo_colegio + '"><i class="ace-icon fa fa-trash-o bigger-130"></i></a>';
                    ...
                    var remove_responsive = '<li><a href="#" data-toggle="modal" data-target="#deleteModal" data-id="' + row.id + '" data-codigo="' + row.codigo_colegio + '" class="tooltip-error" data-rel="tooltip" title="Eliminar"><span class="red"><i class="ace-icon fa fa-trash-o bigger-120"></i></span></a></li>';
...
function btnRemove(id) {
    $.ajax({
        url: "<?= site_url('/Administrador/ajaxRemoverColegio') ?>/" + id,
        type: 'get',                
        dataType: 'json',
        success:function(response) {
            ...
            $.gritter.add({
                        ...
                        text: 'Se ha eliminado con éxito el colegio, código ' + response.data.codigo_colegio + '.',
