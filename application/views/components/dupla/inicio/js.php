        ...
        "columnDefs": [
                			...
                			if(row.tipo == 1){
                				...
                				if(row.tipo == 2){
                					...
                					if (row.tipo == 3){
	                					...
	                					if (row.tipo == 4){
	                						if(row.estado_act == 0) return 'Conductual->Red interna';
                                            if(row.estado_act == 1) return 'Conductual->Red interna (Iniciada)';
	                					} else{ 
	                						if (row.tipo == 5){
	                							if(row.estado_act == 0) return 'Conductual->Red externa';
                                                if(row.estado_act == 1) return 'Conductual->Red externa (Iniciada)';
	                						}
            ...
            },
            {
	            ...
	            "render": function ( data, type, row ) {
	            	...
	            	if(row.tipo == 4){
	            		if(row.estado_act == 0) link_edit ='<?= site_url('derivacionA31/duplaIng') ?>/' + row.id + '/' + row.id_drvz;
                        if(row.estado_act == 1) link_edit ='<?= site_url('derivacionA31/duplaContinuar') ?>/' + row.id + '/' + row.id_drvz;
	            	} 
	            	if(row.tipo == 5){
                        if(row.estado_act == 0) link_edit ='<?= site_url('derivacionA32/duplaIng') ?>/' + row.id + '/' + row.id_drvz;
                        if(row.estado_act == 1) link_edit ='<?= site_url('derivacionA32/duplaContinuar') ?>/' + row.id + '/' + row.id_drvz;
	            	}
