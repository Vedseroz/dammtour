...
$(document).ready(function() {
	...
	.DataTable( {
        ...
        "serverSide": true,
		"ajax": {
            "url": "<?= site_url('Inicio/getEstudiantes') ?>",
            "type": "POST"
        },
        "columnDefs": [
            {
            ...
            },
            {
	            ...
	            "render": function ( data, type, row ) {
	            	...
	            	var link_edit = '*';
                    if(row.etapa == '2') link_edit = '<?= site_url('procedimientosSegundo/Usuario1sp') ?>/' + row.id + '/' + row.id_ctrz;
                    if(row.etapa == '3') link_edit = '<?= site_url('procedimientosTercero/Usuario1sp') ?>/' + row.id + '/' + row.id_ctrz;
                    if(row.etapa == '4') link_edit = '<?= site_url('procedimientosCuarta/comiteIng') ?>/' + row.id + '/' + row.id_ctrz;
                    if(row.etapa == '5') link_edit = '<?= site_url('procedimientosQuinto/Usuario1sp') ?>/' + row.id + '/' + row.id_ctrz;
    ...
    function inicializarDT(){
	   $('#dynamic-table2').DataTable( {
	   	...
	   	"processing": true,
            "serverSide": true,
	   	"ajax": {
                "url": "<?= site_url('Inicio/getEstudiantesDrvz') ?>",
                "type": "POST"
            },
            "columnDefs": [
                {
                ...
                },
                {
                    ...
                    "render": function ( data, type, row ) {
                    			if(row.tipo == 1){
                    				...
                    				if(row.tipo == 2){
                    					if(row.estado_act == 0) return 'Conductual->En espera';
                                        if(row.estado_act == 1) return 'Conductual->En espera (Iniciada)';
                    				}else {
                    					if (row.tipo == 3){
                ...
                },
                {
	               ...
	               "render": function ( data, type, row ) {
	               	...
	               	if(row.tipo == 2){
	               		if(row.estado_act == 0) link_edit ='<?= site_url('derivacionA21/comiteIng') ?>/' + row.id + '/' + row.   id_drvz;
                            if(row.estado_act == 1) link_edit ='<?= site_url('derivacionA21/comiteContinuar') ?>/' + row.id + '/' + row .id_drvz;
	               	}
	               	if(row.tipo == 3){
